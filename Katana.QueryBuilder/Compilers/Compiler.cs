...
namespace Katana.Compilers
{
    public partial class Compiler
    {
        ...
        protected virtual string TranslateColumnType(AbstractCreateClause.Types columnType)
        {
            switch (columnType)
            {
                case AbstractCreateClause.Types.AutoId:
                    ...
                    string key = $"{guid}_pk";
                    return $"int IDENTITY CONSTRAINT {Wrap(key)} PRIMARY KEY";
        ...
        protected virtual SqlResult CompileRaw(Query query)
        {
            ...
            else if (query.Method == "create")
            {
                ctx = CompileCreateQuery(query);
            }
            else
            {
        ...
        protected virtual SqlResult CompileCreateQuery(Query query)
        {
            var ctx = new SqlResult
            {
                Query = query
            ...
            {
                string columnQueryPart = $"{Wrap(column.Name)} {TranslateColumnType(column.Type)}";
                if (column.NotNull) columnQueryPart += " NOT NULL";
                if (column.Unique)
                {
                    string indexName = Wrap($"{create.Name}_{column.Name}_uindex");
                    postCommands.Add(
                        $"CREATE UNIQUE INDEX {indexName} on {Wrap(create.Name)} ({Wrap(column.Name)}); ");
                }

                columns.Add(columnQueryPart);
            }

            ctx.RawSql = $"CREATE TABLE {Wrap(create.Name)} ({string.Join(", ", columns)}); ";
            foreach (string postCommand in postCommands) ctx.RawSql += postCommand;
